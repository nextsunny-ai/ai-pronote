...
test.describe('버전 표시와 자동 업데이트 안내', () => {
  test('새 버전이 있으면 팝업을 띄우고 명시적 선택 뒤에만 준비한다', async ({ page }) => {
    ...
    await page.route('**/api/health', route => route.fulfill({
      status: 200, contentType: 'application/json', body: JSON.stringify({ status: 'ok', version: 'v1.5.0-beta13.20260920.3' })
    }));
    ...
    await openApp(page);
    await expect(page.locator('#appVersionLabel')).toContainText('v1.5.0-beta13.20260920.3');
