...
for CANDIDATE in "${PYTHON_CANDIDATES[@]}"; do
  if command -v "$CANDIDATE" >/dev/null 2>&1 && "$CANDIDATE" -c 'import platform,struct,sys; assert sys.version_info[:2] == (3,12); assert struct.calcsize("P")*8 == 64 and platform.machine().lower() in {"arm64","x86_64"}' >/dev/null 2>&1; then
    PYTHON="$(command -v "$CANDIDATE")"
    break
  fi
done
if [[ -z "$PYTHON" ]]; then
  echo "Python 3.12 64비트가 필요합니다. 공식 다운로드 페이지를 엽니다."
...
".venv/bin/python" -c 'import platform,struct,sys; machine=platform.machine().lower(); assert sys.version_info[:2] == (3,12), "기존 .venv는 Python 3.12 환경이어야 합니다"; assert struct.calcsize("P")*8 == 64 and machine in {"arm64","x86_64"}, f"기존 .venv가 지원 Mac 64비트 환경이 아닙니다: {machine}"'
