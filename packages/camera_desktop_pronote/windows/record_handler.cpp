...
static HRESULT BuildVideoOutputType(IMFMediaType* base_type,
  ...
  // CopyAllItems above already preserved the camera's exact frame-rate ratio
  // (for example 30000/1001). Replacing it with a rounded integer gradually
  // desynchronizes the Media Foundation record sink and produces duplicate or
  // non-monotonic DTS values during long recordings.
  if (bitrate > 0) {
